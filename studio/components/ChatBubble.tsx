...
export function ChatBubble({
  ...
  // Tick "now" once on mount plus every 30s so the relative timestamp updates
  ...
  if (message.error) {
    return (
      <div className="min-w-0 max-w-full break-words rounded-md border border-danger/40 bg-danger/10 px-3 py-2 text-sm text-danger [overflow-wrap:anywhere]">
        {message.error}
      </div>
    );
  }
  ...
  return (
    ...
    <div
      className={cn(
        "group flex w-full min-w-0 max-w-full flex-col gap-1 overflow-x-hidden",
    ...
    >
      {isUser ? (
        ...
        // accent plus label so the transcript shows that they were typed
        // mid-turn and routed through the steer channel.
        // No-bubble user message: text reads naturally left-to-right but
        // the whole block hugs the column's right edge. max-w-[85%] gives
        // long messages more room to breathe than the old 78% while
        // keeping the wrap point tight enough for scanning.
        ...
        // reply" even after the ThinkingBlock has collapsed.
        <div className="flex w-full min-w-0 max-w-full items-start gap-2 overflow-x-hidden">
          {message.pending && (
            <span
          ...
          {/* Agent bubble: a touch darker than the surrounding muted
              ...
              Proactive bubbles get a subtle accent border plus origin badge. */}
          <div
            className={cn(
              "min-w-0 max-w-full overflow-hidden rounded-2xl rounded-tl-sm bg-zinc-200/80 px-3 py-2 text-sm leading-relaxed text-foreground [overflow-wrap:anywhere] sm:max-w-[80%] dark:bg-zinc-800/80",
      ...
      {/* Metadata row: timestamp stays visible all the time. Action icons
          live in a child container that fades in on hover and is always
          visible on touch devices via hover:none media. */}
      {!message.pending && (
        <div
          className={cn(
            ...
            // user's message text. Agent row keeps its small left inset.

...
| `Inset` | `ui/inset.tsx` | `{ variant?: "plain" \| "terminal" \| "diff" \| "quote" \| "kv" \| "schema" }`. Tinted `bg-muted` radius-10 block, never bordered. Owns `min-w-0 max-w-full overflow-wrap:anywhere`. `terminal` renders `$ cmd` lines + trimmed output with "Show all"; `diff` reuses the existing `DiffPre` line tinting; `quote` sets voice with curly quotes; `kv` is a `dt/dd` grid; `schema` is a mono table of fields. |

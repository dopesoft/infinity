...
| `/ws` | WS | server.handleWebSocket | streaming chat protocol. Server→client: `delta/thinking/tool_call/tool_input_delta/tool_result/effort/steer_received/complete/error` (every one stamped `seq` + `turn_id` by the per-session **turn journal**, `turn_journal.go`), `turn_status` (answer to `attach`/`interrupt`: in_flight, phase, started_at, replay floor) and `heartbeat` (same payload every 5s while a turn runs). Client→server: `attach {session_id, since_seq}` on every connect and session switch (binds the socket; the journal replays frames past `since_seq` flagged `replay`), `message`, `steer`, `resume`, `interrupt` (always answered), `ping`, `clear`. One writer goroutine per socket with a kind-based drop policy (`ws_conn.go`); sessions fan out to every bound socket (`proactive_wire.go`). Studio's `lib/chat/liveness.ts` is the only client logic that acts on silence (20s → attach, 45s → reconnect, never an error card). **Identity, not text:** the turn id is minted once in `startTurn` and rides the ctx (`turnctx.WithTurnID`) into the `mem_turns` row; every `delta` carries `msg_index` (the loop's `Session.replyIndex`) and every persisted assistant row carries the same `message_index`; `message`/`steer` carry the browser's `client_id`, echoed on `steer_received` and returned on the transcript row. `lib/chat/transcript.ts` merges by `reply:<turn>:<index>` / `user:<client_id>` / `tool:<call_id>` / `row:<obs id>`, never by prefix. **Budget:** `turn_budget.go` cancels a turn with a cause: `ErrTurnStalled` after `INFINITY_TURN_STALL` (10m) of silence while thinking/streaming, `ErrTurnCeiling` after `INFINITY_TURN_TIMEOUT` (2h); a silent tool or parked approval never stalls. `ws_e2e_test.go` drives the real handler over a real socket through the `Server.runLoop` seam. **The last thing Jarvis said in a turn is his reply:** the loop files pre-tool-call messages `interim:true` (folded into the ledger), so `GET /api/sessions/{id}/messages` (`promoteLastReply`) un-folds the last assistant row of any finished turn that has no `TaskCompleted` (15s grace for the async insert), the loop persists a reply cut by a provider error (`persistCutReply`), and Studio's `settle.ts promoteLastReply` does the same live on complete/error. |
...
| 8 | Voice | ✅ **GPT Realtime over WebRTC** — `core/internal/voice/realtime.go` mints short-lived OpenAI `client_secret`s; the browser does the WebRTC SDP exchange P2P with `api.openai.com` (audio never touches Core); tool calls round-trip through `/api/voice/tool` so voice has the same registry + Trust gate as text, and `/api/voice/turn` fires the same memory-capture hooks. Model `gpt-realtime-2.1-mini`, server-VAD barge-in, live transcription. nil-safe — `/api/voice/*` returns 503 when `OPENAI_API_KEY` is unset. | Studio mic-button polish · wake-word activation (currently tap-to-talk) |

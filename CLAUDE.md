...
### Context economy — the prefix never moves, and nothing is sent twice

**Audited 2026-09-04 against the live database and the Claude Code session files on the Mac.** The design (stable/volatile split, active set + catalog, `tool_search`/`load_tools`, session resume, compaction to memory) was right; the wiring re-sent ~64K chars of context on every LLM call, moved it every iteration so each tool round trip re-wrote the whole turn, mutated the tool array mid-turn, and on the Claude brain clamped the init line that carries the session id so every chat turn started cold (54 sessions on the Mac, one per turn). The rules below are how Anthropic's and OpenAI's own agents (Claude Code, Codex CLI, the Messages API context-editing and compaction features, the Responses compaction endpoint) do it, and they are enforced in code, not in prose.

- **Per-turn context is PINNED to the user message that opened the turn** (`llm.Message.Volatile`, rendered by every provider as that message's trailing block; `Session.pinVolatile` / `clearVolatile` in [`session_context.go`](core/internal/agent/session_context.go)). It never rides the tail of the request. Within a turn every call renders identical bytes up to the newest message; `llm/pinned_context_test.go` pins this per provider. `SystemPrompt.Volatile` is now only the rare per-call overlay (a wind-down notice that could not ride the newest tool result).
- **World state is sent once, then as diffs** ([`worldstate.go`](core/internal/agent/worldstate.go)): the tool catalog, connected accounts, bridge, installed CLI tools, compass and goals are lifted out of the memory prefix by tag, sent as ONE user-role message on the session's first turn (it stays in the history and becomes cached prefix), and afterwards only a `<world_state_update>` for sections whose hash changed. A provider that wants its block treated this way wraps it in a tag listed in `worldSectionTags`. Never re-add a stable overlay to the volatile builder. Self-executing brains (Claude Code) get no catalog at all; they carry their own deferred tool list.
- **The tool array and effort are frozen for the turn.** `DecayTTL()` runs once at turn start, never per call. A `load_tools` mid-turn is one deliberate prefix rebuild; a TTL expiring mid-turn was dozens. Effort is resolved once per turn (`resolveEffort`) and never changed by a later pass.
- **Context maintenance runs before every call on the MEASURED fill** ([`context_maint.go`](core/internal/agent/context_maint.go)), in the vendors' order: at 50% of the window, tool results older than the newest 3 become one-line markers (only when ≥40K chars would be freed, so the prefix rewrite is worth it); at 60%, compaction. Compaction is token-gated (`Force`), works INSIDE a single long turn (`compactInTurn`, newest 3 exchanges kept verbatim), writes the summary as a **user** message (a `RoleSystem` message mid-transcript is silently dropped by every provider), and runs the summariser against the session's own cached prefix with tool calls disabled (`llm.WithNoToolCalls`) so it reads the warm cache. On a brain that holds the conversation itself, compaction drops its session (`llm.ForgetSessionIfSupported`). Manual: `POST /api/sessions/{id}/compact`, booked as a `session.compact` run, offered on the context meter above 25% fill.
- **Every LLM call logs its cache split** (`llm-call: … prompt= cached= cache_write= uncached=`). A turn whose second call is not ≥90% cached is a cache break; treat it as an incident, not as weather.
- **Trim at the boundary, once.** MCP tool descriptions are cut to their first paragraph (≤600 chars) at registration; Infinity MCP results handed to Claude Code pass through the same `TrimToolResult` the loop uses and advertise the cap in `_meta`; the Honcho representation is deduped by fact and capped at 3K chars; `<gym_reflex>` at 3K; attachments older than the newest message drop their bytes.
- **Claude brain:** the init line is read with its own `head -c 32000` and parsed tolerantly; a cold start on a conversation with history is logged loudly (`COLD start`); `--max-turns` is the segment's tool budget; `--bare` is NOT used (verified: it drops the Mac's sign-in). The continuation poller never replays into the live conversation.

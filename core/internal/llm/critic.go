...
- "fix": include this object ONLY when the session hit a CONCRETE, CODE-FIXABLE blocker — a tool/recipe/config/routing bug that a code change would prevent (e.g. a tool returning 413 because it doesn't paginate, the agent invoking a skill through the wrong tool, a sub-agent using a model the account rejects). Name the real artifact in "title"/"change", and put a CONCRETE code location in "target_hint" — a file path ("core/internal/llm/openai_oauth.go") or a symbol ("classifyOutcome"). OMIT "fix" entirely (or null) for user-side issues, transient outages (429/503), or sessions that went fine. This is how a noticed failure becomes a queued repair — only emit it when a code change is the actual remedy.
- DO NOT propose a fix for behavior the system ALREADY enforces deterministically in code: loop/repeat limits, gates and approvals, retries, dedupe, "never send / never auto-dismiss", or bridge failover. If your remedy is "a skill should say X" or "add a rule so Y can't happen" and Y is the kind of thing code guards (a repeated tool call, a destructive command, a swallowed error), that mechanic is ALREADY handled — OMIT the fix. A vague target like "delegate model routing" or "the post-deploy-verify skill" is the smell of this mistake: if you cannot name a specific file or symbol that is genuinely missing or wrong, there is no code fix to queue. Proposing one is a false alarm, and false alarms are worse than silence.

...
func (a *API) planWorkItems(ctx context.Context) ([]WorkItem, error) {
	...
	for _, p := range plans {
		...
		// Whether a non-terminal plan is REALLY still running is not decided
		// here. Six producers used to answer that question six ways and four
		// of them got it wrong; it is one rule now, in liveness.go, applied to
		// every item from every producer at the seam where the board is built.

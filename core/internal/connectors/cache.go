...
func (c *Cache) SystemPromptBlock() string {
	...
	// NOTE: do not mention the old MCP gateway tools (COMPOSIO_SEARCH_TOOLS /
	// COMPOSIO_MULTI_EXECUTE_TOOL) here — the Composio MCP leg is disabled
	// (vendor moved it to AuthKit-only auth; see config/mcp.yaml), so those
	// tools no longer register and naming them sends the brain chasing ghosts.
	b.WriteString("The boss has authenticated the following SaaS accounts via Composio. Every verb of each connected toolkit is already in your <tool_catalog> as `composio__TOOLKIT_VERB` - discover with `tool_search(\"send gmail\")`, bring online with `load_tools([\"composio__GMAIL_SEND_EMAIL\"])`, then call it directly. If a verb you expect is missing from the catalog, the toolkit's verbs have not registered yet - check the account with `connector_accounts_list` rather than hunting for gateway fallbacks.\n\n")

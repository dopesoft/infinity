...
func TestApplyComposioAuthUsesProjectRESTHeaders(t *testing.T) {
	...
	if got := req.Header.Get("Authorization"); got != "Bearer project-key" {
		t.Fatalf("Authorization = %q, want Bearer project-key", got)
	}
...
func TestComposioToolkitsProxyUsesProjectAPIKey(t *testing.T) {
	...
	var sawPath, sawAPIKey, sawConsumerKey string
	...
	if sawPath != "/api/v3/toolkits" {
		t.Fatalf("proxied path = %q, want /api/v3/toolkits", sawPath)
	}
...
func TestComposioConnectWorksWithProjectAPIKey(t *testing.T) {
	...
	withComposioHTTP(t, roundTripFunc(func(r *http.Request) (*http.Response, error) {
		...
		case "GET /api/v3/auth_configs":
			return jsonResp(http.StatusOK, `{"items":[{"auth_config":{"id":"ac_123"}}]}`), nil
		case "POST /api/v3/connected_accounts/link":
	...
	if got, want := strings.Join(calls, ","), "GET /api/v3/auth_configs,POST /api/v3/connected_accounts/link"; got != want {

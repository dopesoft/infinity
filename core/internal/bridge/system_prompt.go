...
type MemoryProvider struct {
	Router *Router
	Prefs  PrefFetcher
	// mac caches the Mac's real home + repo list so the overlay can state
	// them as fact instead of leaving the model to invent `/Users/kai`.
	mac factsCache
}
...
func (m *MemoryProvider) BuildSystemPrefix(ctx context.Context, sessionID, query string) (string, error) {
	...
	case active.Name() == KindMac:
		...
		b.WriteString("Coding muscle: for any non-trivial coding task, DELEGATE it with `code_agent` - that runs `claude -p` (the real Claude Code agent) on the Mac under the boss's Anthropic Max subscription, so the *coding cognition* is Max-billed and does NOT spend your own (chat-model) quota. You write the brief, Claude Code writes the code. Do NOT hand-author code with `claude_code__Edit`/`claude_code__Write`/`fs_edit` for real work - those are dumb file-writers, so YOU end up authoring every byte against your own quota (this is the bug that was burning the boss's ChatGPT plan on the Mac bridge). Reserve direct `fs_edit`/`claude_code__Edit` for tiny one-line or deterministic single-shot changes. `code_agent` runs freely; only filesystem deletes are blocked and surfaced for the boss to approve. `background_build` on the Mac also runs on Claude Code. Every run proves it is signed in to his Max subscription before it starts; if it reports his Claude plan is out of usage, tell him and wait - never move the coding onto your own plan.\n")
		b.WriteString(m.mac.facts(ctx, active).line())
		b.WriteString("Filesystem: your shell starts in `~/Dev`, the umbrella folder that holds EVERY repo (infinity, ELMAGO, 360PaintKing, ...). `~/Dev` is NOT itself a git repository, so a bare `git status`/`git diff`/`git commit` run there fails with exit 128 (\"not a git repository\"). ALWAYS `cd` into the specific repo first - e.g. `cd ~/Dev/infinity && git status` - before any git, build, or test command. Commits are authored as the boss's git identity.\n")
		b.WriteString("Failover: if a `claude_code__*` call returns `mac_bridge_unavailable`, the Mac just went offline mid-turn - do NOT retry it. `load_tools` the cloud primitives (`fs_edit`/`fs_save`/`bash_run`/`git_*`) and continue the work on `/workspace` (`git pull` that repo first); the next turn auto-routes to Cloud and hides the `claude_code__*` schemas.\n")
